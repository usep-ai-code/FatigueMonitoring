...
const SCCDashboard = () => {
  ...
  useEffect(() => {
    ...
    const streamUrl = `${apiBaseUrl}/api/dashboard/stream`
  ...
  useEffect(() => {
    ...
    if (!hasLoadedRef.current) {
      previousAlertIdsRef.current = new Set(alerts.map((alert) => alert.alarmId))
      hasLoadedRef.current = true
      return
    }

    const newIds = new Set(alerts.map((alert) => alert.alarmId))
    alerts.forEach((alert) => {
      if (!previousAlertIdsRef.current.has(alert.alarmId)) {
  ...
  const parseOpenedAt = (openedAt) => (openedAt ? new Date(openedAt) : null)

  const getOpenDurationValue = (openedAt, fallbackMinutes) => {
    const openedDate = parseOpenedAt(openedAt)
    if (!openedDate) return fallbackMinutes || 0
    const diffMs = currentTime - openedDate
    const diffMins = Math.floor(diffMs / 60000)
    return diffMins < 0 ? 0 : diffMins
  }

  const formatTime = (openedAt) => {
    const openedDate = parseOpenedAt(openedAt)
    if (!openedDate) return '--:--:--'
    return openedDate.toLocaleTimeString('id-ID', { hour12: false })
  }
  ...
  const kpisByArea = useMemo(() => {
    ...
    snapshot.kpis?.forEach((kpi) => {
      base[kpi.area] = {
        totalAlarms: kpi.totalAlarms,
        followedUp: kpi.followedUp,
        waitingFollowUp: kpi.waitingFollowUp,
      }
    })
    return base
  }, [snapshot.kpis])
  ...
  const stats = useMemo(() => {
    const kpi = kpisByArea[selectedArea] || kpisByArea.All
    return {
      totalToday: kpi.totalAlarms,
      followedUpToday: kpi.followedUp,
      activeOpen: kpi.waitingFollowUp,
    }
  }, [kpisByArea, selectedArea])

  const areaSummary = useMemo(() => ({
    Mining: {
      total: kpisByArea.Mining.totalAlarms,
      open: kpisByArea.Mining.waitingFollowUp,
      resolved: kpisByArea.Mining.followedUp,
    },
    Hauling: {
      total: kpisByArea.Hauling.totalAlarms,
      open: kpisByArea.Hauling.waitingFollowUp,
      resolved: kpisByArea.Hauling.followedUp,
    },
  }), [kpisByArea])

  const locationStats = useMemo(() => {
    const statsMap = { Mining: {}, Hauling: {} }
    snapshot.areaDistributions?.forEach((item) => {
      if (!statsMap[item.area]) {
        statsMap[item.area] = {}
      }
      statsMap[item.area][item.location] = item.openCount
    })
    return statsMap
  }, [snapshot.areaDistributions])

  const filteredAlertsByArea = useMemo(() => {
    return selectedArea === 'All'
      ? snapshot.activeAlerts
      : snapshot.activeAlerts.filter((alert) => alert.area === selectedArea)
  }, [snapshot.activeAlerts, selectedArea])
  ...
  }, [filteredAlertsByArea, selectedLocationFilter])

  const overdueAlerts = useMemo(() => {
    const delayed = snapshot.delayedAlerts || []
    return selectedArea === 'All'
      ? delayed
      : delayed.filter((alert) => alert.area === selectedArea)
  }, [snapshot.delayedAlerts, selectedArea])

  const highRiskOperators = useMemo(() => {
    const units = snapshot.recurrentUnits || []
    return selectedArea === 'All'
      ? units
      : units.filter((unit) => unit.area === selectedArea)
  }, [snapshot.recurrentUnits, selectedArea])

  const highFreqZones = useMemo(() => {
    const zones = snapshot.highRiskAreas || []
    return selectedArea === 'All'
      ? zones
      : zones.filter((zone) => zone.area === selectedArea)
  }, [snapshot.highRiskAreas, selectedArea])

  const deviceHealth = snapshot.deviceHealth || {
    totalDevices: 0,
    onlineDevices: 0,
    offlineDevices: 0,
    coverage: 0,
  ...
  const PaginationControls = ({ currentPage, totalPages, onPageChange }) => {
    ...
    return (
      <div
        className={`flex items-center justify-end gap-3 mt-auto pt-2 border-t border-dashed ${
          darkMode ? 'border-slate-700' : 'border-slate-300'
        }`}
      >
        <span
          className={`text-[clamp(0.8rem,1.2vh,1rem)] font-medium ${
  ...
  const statusConfig = STATUS_CONFIG[sseStatus] || STATUS_CONFIG.connecting
    ...
    >
      <div className="absolute top-[10vh] right-[2vw] z-[100] flex flex-col gap-4 pointer-events-none max-w-[600px] w-[30vw]">
        {notifications.map((notif) => (
          <div
            key={notif.id}
            className={`pointer-events-auto p-6 rounded-xl shadow-2xl border-l-8 flex items-start gap-4 w-full animate-in slide-in-from-right duration-300 ${
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p
                className={`text-lg mt-1 break-words leading-snug ${
                  darkMode ? 'text-slate-200' : 'opacity-80'
                }`}
              >
                {notif.message}
              </p>
      ...
      {selectedAlert && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
          <div
            className={`w-full max-w-[90vw] h-[85vh] rounded-3xl shadow-2xl overflow-hidden flex flex-col ${
            ...
            <div className="p-[3vh] border-b border-inherit flex justify-between items-start shrink-0">
              <div>
                ...
                <p className={`text-2xl ${darkMode ? 'text-slate-300' : 'opacity-70'}`}>
                  Unit
                  <span className="font-mono font-bold bg-slate-700 text-white px-3 py-1 rounded mx-2">
                    {selectedAlert.unit}
                  </span>
            ...
            <div className="flex-1 p-[3vh] overflow-hidden">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-[4vh] h-full">
                <div className="flex flex-col gap-[2vh] h-full">
                  <h3
                    className={`font-bold text-xl uppercase tracking-wider flex items-center gap-3 shrink-0 ${
                      darkMode ? 'text-slate-200' : 'opacity-70'
                    }`}
                  >
                  ...
                  <div className="flex-1 relative rounded-2xl bg-black border-4 border-slate-700 overflow-hidden group">
                    <div className="absolute inset-0 bg-green-900/10 z-10 pointer-events-none mix-blend-overlay" />
                    <div className="absolute inset-0 bg-[radial-gradient(circle,transparent_40%,#000_100%)] z-20 pointer-events-none" />
                    <div className="absolute top-[25%] left-[30%] right-[30%] bottom-[25%] border-4 border-red-500/80 rounded-xl z-30 shadow-[0_0_20px_rgba(239,68,68,0.5)] animate-pulse flex flex-col items-center justify-end pb-8">
                      <div className="bg-red-600 text-white text-xl font-bold px-6 py-3 rounded-lg flex items-center gap-3 shadow-lg">
                        <EyeOff size={24} />
                        {selectedAlert.alarmType || 'FATIGUE'} DETECTED
                      </div>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4 shrink-0 h-[12vh]">
                    ...
                    >
                      <span
                        className={`text-sm block uppercase tracking-wider font-bold mb-1 ${
                          darkMode ? 'text-slate-300' : 'opacity-60'
                        }`}
                      >
                        Vehicle Speed
                      </span>
                      <span className="font-mono font-bold text-5xl">
                        {selectedAlert.speedKph !== null && selectedAlert.speedKph !== undefined
                          ? `${Math.round(selectedAlert.speedKph)} km/h`
                          : '--'}
                      </span>
                    </div>
                    <div
                    ...
                    >
                      ...
                      <span className="font-bold text-4xl text-red-500">
                        {selectedAlert.alarmType || 'Fatigue'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-[2vh] h-full">
                  ...
                  <div
                    className={`flex-1 relative rounded-2xl border border-slate-600 overflow-hidden ${
                  ...
                  >
                    ...
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center group cursor-pointer">
                      <div className="w-32 h-32 rounded-full bg-red-500/20 animate-ping absolute top-0" />
                      <div className="relative z-10 text-red-600 drop-shadow-xl transform group-hover:-translate-y-2 transition-transform">
                        <MapPin size={96} fill={darkMode ? '#ef4444' : '#dc2626'} className="text-white" />
                      </div>
                      <div className="mt-4 bg-slate-900 text-white text-xl px-6 py-3 rounded-xl shadow-lg font-bold whitespace-nowrap z-20">
                        {selectedAlert.location}
                      </div>
                    </div>
                  </div>
                  <div
                    className={`p-6 rounded-xl border shrink-0 flex items-center justify-between ${
                      darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-300'
                    }`}
                  >
                    <div>
                      <span className={`text-sm block font-bold ${darkMode ? 'text-slate-300' : 'opacity-60'}`}>
                        GPS Coordinates
                      </span>
                      <span className="font-mono text-2xl font-medium text-blue-500">
                        Lat: {selectedAlert.latitude ?? '--'}, Long: {selectedAlert.longitude ?? '--'}
                      </span>
                      <div className={`text-sm mt-2 ${darkMode ? 'text-slate-400' : 'opacity-50'}`}>
                        Accuracy: ±2m
                      </div>
                    </div>
      ...
      <header
        className={`h-[8vh] border-b flex items-center justify-between px-[2vw] shrink-0 ${
      ...
      >
        <div className="flex items-center gap-6">
          <div className="hidden lg:block border-r pr-8 mr-2 border-inherit">
            <img src="/alamtri-logo.svg" alt="Alamtri Logo" className="h-[4vh] object-contain" />
          </div>
          <div className="p-3 bg-red-600 rounded-xl shadow-lg shadow-red-500/30">
            <ShieldAlert className="w-[3vh] h-[3vh] text-white" />
          </div>
          <div>
            <h1 className="text-[clamp(1.5rem,2vh,2.5rem)] font-black tracking-tight leading-tight">
              FATIGUE COMMAND CENTER
            </h1>
            <p
              className={`text-[clamp(1rem,1.2vh,1.5rem)] font-medium ${
                darkMode ? 'text-slate-200' : 'text-slate-500'
              }`}
            >
        ...
        <div className="flex items-center gap-[2vw]">
          <div
            className={`hidden md:flex items-center gap-6 px-[1.5vw] py-[1vh] rounded-full border ${
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-3 h-3 lg:w-4 lg:h-4 rounded-full ${
                  deviceHealth.offlineDevices === 0 ? 'bg-emerald-500' : 'bg-amber-500 animate-pulse'
                }`}
              />
              <span
                className={`text-[clamp(1rem,1.2vh,1.5rem)] font-semibold whitespace-nowrap ${
            ...
            <div className="h-6 w-px bg-slate-600/30" />
            <div className="flex items-center gap-4 text-[clamp(1rem,1.2vh,1.5rem)]">
              <div className="flex items-center gap-2">
                <Wifi className="w-[2.5vh] h-[2.5vh] text-emerald-500" />
                <span className={`font-mono font-bold ${darkMode ? 'text-white' : ''}`}>
                  {deviceHealth.onlineDevices}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <WifiOff className="w-[2.5vh] h-[2.5vh] text-red-500 ml-1" />
                <span className={`font-mono font-bold ${darkMode ? 'text-white' : ''}`}>
          ...
          <div
            className={`flex items-center gap-3 px-[1vw] py-[0.6vh] rounded-full border ${
              darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-300 shadow-sm'
            }`}
          >
            <span className={`w-3 h-3 rounded-full ${statusConfig.color} ${statusConfig.pulse ? 'animate-pulse' : ''}`} />
            <span className={`text-[clamp(0.9rem,1.2vh,1.3rem)] font-semibold ${
              darkMode ? 'text-slate-200' : 'text-slate-700'
            }`}>
          ...
          <div className={`flex flex-col items-end ${darkMode ? 'text-white' : 'text-slate-600'}`}>
            ...
            <span
              className={`text-[clamp(0.9rem,1.2vh,1.3rem)] font-medium uppercase tracking-wider mt-1 ${
          ...
          <button
            ...
            className={`p-[1vh] rounded-full transition-colors ${
      ...
      <main className="flex-1 flex p-[2vh] gap-[2vh] overflow-hidden min-h-0">
        <div className="flex-[4] flex flex-col gap-[2vh] min-w-0 h-full">
          <div className="flex gap-[2vh] shrink-0 h-[16vh] min-h-[140px]">
            <div className="flex-[1.8] flex gap-[2vh]">
              <div className={`flex-1 p-[2vh] rounded-2xl border flex items-center justify-between ${getCardBg()}`}>
                <div className="flex flex-col justify-center h-full">
                  <p
                    className={`text-[clamp(1rem,1.4vh,1.6rem)] uppercase font-bold tracking-wider mb-2 ${
                      darkMode ? 'text-slate-200' : 'text-slate-500'
                    }`}
                  >
                  ...
                  <h2
                    className={`text-[clamp(3rem,5vh,6rem)] font-black leading-tight ${
                ...
                <div
                  className={`p-[1.5vh] rounded-full ${
              ...
              <div className={`flex-1 p-[2vh] rounded-2xl border flex items-center justify-between ${getCardBg()}`}>
                <div className="flex flex-col justify-center h-full">
                  ...
                  <h2 className="text-[clamp(3rem,5vh,6rem)] font-black leading-tight text-emerald-500">
            ...
            <div
              className={`flex-1 p-[2vh] rounded-2xl border flex items-center justify-between relative overflow-hidden ${
            ...
            >
              <div className="z-10 flex flex-col justify-center h-full">
                ...
                <h2 className="text-[clamp(3.5rem,6vh,7rem)] font-black leading-tight text-red-600">
                  {stats.activeOpen}
                </h2>
              </div>
              ...
              <div className="absolute -right-4 -bottom-4 w-[15vh] h-[15vh] bg-red-500/10 rounded-full blur-xl" />
            </div>
          </div>
          ...
          <div className="flex-[1.5] flex gap-[2vh] min-h-0">
            <div className={`flex-[1.8] rounded-2xl border flex flex-col p-[2vh] overflow-hidden ${getCardBg()}`}>
              <div className="flex items-center mb-[1.5vh] shrink-0">
                <div
                  className={`px-4 py-2 rounded-lg text-[clamp(1rem,1.5vh,1.8rem)] font-bold flex items-center gap-3 ${
                ...
                >
                  ...
                  <span className="w-3 h-3 rounded-full bg-blue-500 animate-pulse ml-2" />
                </div>
              </div>
                    ...
                    <div className={`rounded-xl p-[1.5vh] flex flex-col h-full ${darkMode ? 'bg-slate-900/40' : 'bg-white/60 border border-slate-200'}`}>
                      <div className={`flex items-center justify-between mb-2 border-b pb-2 shrink-0 ${darkMode ? 'border-slate-700' : 'border-slate-200'}`}>
                        ...
                        <div className="flex items-center gap-3 text-[clamp(0.8rem,1vh,1.2rem)]">
                          <span className={darkMode ? 'text-slate-200 font-bold' : 'text-slate-600'}>
                            {areaSummary.Mining.total} Total
                          </span>
                      ...
                      <div className="flex-1 flex flex-col justify-between overflow-hidden" ref={miningListContainerRef}>
                        <div className="grid grid-cols-2 gap-3 mt-1">
                          {Object.keys(locationStats.Mining).length > 0 ? (
                            paginate(Object.entries(locationStats.Mining), miningPage, dynamicItemsPerPage.mining).map(
                              ([location, count]) => (
                                <div
                                  key={location}
                                  onClick={() => {
                                  ...
                                  className={`p-3 rounded-xl border flex flex-col justify-between items-center text-center transition-all cursor-pointer hover:scale-105 ${
                                ...
                                >
                                  <span className={`text-[clamp(0.9rem,1.2vh,1.4rem)] font-medium mb-1 line-clamp-1 ${darkMode ? 'text-white' : 'text-slate-600'}`}>
                                    {location}
                                  </span>
                                  <span className="text-[clamp(1.8rem,2.5vh,3rem)] font-black text-red-500 leading-none">
                                    {count}
                                  </span>
                                </div>
                              ),
                        ...
                        <PaginationControls
                          currentPage={miningPage}
                          totalPages={Math.ceil(Object.keys(locationStats.Mining).length / dynamicItemsPerPage.mining)}
                          onPageChange={setMiningPage}
                        />
                      </div>
                    </div>
                  )}
                  ...
                  {(selectedArea === 'All' || selectedArea === 'Hauling') && (
                    <div className={`rounded-xl p-[1.5vh] flex flex-col h-full ${darkMode ? 'bg-slate-900/40' : 'bg-white/60 border border-slate-200'}`}>
                      <div className={`flex items-center justify-between mb-2 border-b pb-2 shrink-0 ${darkMode ? 'border-slate-700' : 'border-slate-200'}`}>
                        ...
                        <div className="flex items-center gap-3 text-[clamp(0.8rem,1vh,1.2rem)]">
                          <span className={darkMode ? 'text-slate-200 font-bold' : 'text-slate-600'}>
                            {areaSummary.Hauling.total} Total
                          </span>
                      ...
                      <div className="flex-1 flex flex-col justify-between overflow-hidden" ref={haulingListContainerRef}>
                        <div className="grid grid-cols-2 gap-3 mt-1">
                          {Object.keys(locationStats.Hauling).length > 0 ? (
                            paginate(Object.entries(locationStats.Hauling), haulingPage, dynamicItemsPerPage.hauling).map(
                              ([location, count]) => (
                                <div
                                  key={location}
                                  onClick={() => {
                        ...
                        <PaginationControls
                          currentPage={haulingPage}
                          totalPages={Math.ceil(Object.keys(locationStats.Hauling).length / dynamicItemsPerPage.hauling)}
                          onPageChange={setHaulingPage}
                        />
                      </div>
                    </div>
                  )}
            ...
            <div
              className={`flex-1 rounded-2xl border-2 flex flex-col p-[2vh] overflow-hidden ${
                darkMode ? 'bg-red-900/10 border-red-500 shadow-[0_0_20px_rgba(239,68,68,0.3)]' : 'bg-red-50 border-red-600 shadow-md'
              }`}
            >
              ...
              <div className="flex-1 flex flex-col justify-between overflow-hidden">
                  ...
                  {overdueAlerts.length > 0 ? (
                    paginate(overdueAlerts, delayedPage, ITEMS_DELAYED).map((alert) => (
                      <div
                        ...
                        className={`relative group p-3 rounded-xl border flex justify-between items-center cursor-pointer transition-all hover:bg-red-500/10 hover:border-red-400 ${
                          darkMode ? 'bg-slate-900 border-red-500/30' : 'bg-white border-red-200 shadow-sm'
                        }`}
                      >
                        <div>
                          <div className={`font-bold text-[clamp(1rem,1.2vh,1.4rem)] ${darkMode ? 'text-white' : 'text-slate-900'}`}>
                            {alert.unit}
                          </div>
                          <div className={`text-[1rem] ${darkMode ? 'text-slate-300' : 'text-slate-500'}`}>
                            {alert.location}
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="text-[1.2rem] font-black text-red-500 font-mono">
                            +{getOpenDurationValue(alert.openedAt, alert.openMinutes)}m
                          </div>
                ...
                <PaginationControls
                  ...
                  totalPages={Math.ceil(overdueAlerts.length / ITEMS_DELAYED)}
                  onPageChange={setDelayedPage}
                />
              </div>
            </div>
          </div>
          ...
          <div className="flex-[1.2] flex gap-[2vh] min-h-0">
            <div className={`flex-1 rounded-2xl border p-[2vh] overflow-hidden flex flex-col ${getCardBg()}`}>
              <div className="flex items-center justify-between mb-[1.5vh] shrink-0">
                ...
                <span className="bg-red-600 text-white text-[1rem] font-bold px-3 py-1 rounded-full">
              ...
              <div className="flex-1 flex flex-col justify-between overflow-hidden" ref={recurrentListContainerRef}>
                <div className="flex-1 space-y-2">
                  {highRiskOperators.length > 0 ? (
                    paginate(highRiskOperators, recurrentPage, dynamicItemsPerPage.recurrent).map((op, idx) => (
                      <div key={`${op.unit}-${idx}`} className={`px-3 py-2 rounded-xl border-l-4 border-red-500 flex justify-between items-center ${darkMode ? 'bg-slate-900/50' : 'bg-white border border-slate-200'}`}>
                        <div>
                          <div className={`font-bold text-[clamp(1rem,1.2vh,1.4rem)] ${darkMode ? 'text-white' : 'text-slate-800'}`}>
                            {op.unit}
                          </div>
                          <div className={`text-[0.9rem] ${darkMode ? 'text-slate-300' : 'text-slate-500'}`}>
                            {op.operator} • <span className="text-red-500 font-semibold">{op.eventsCount} events</span>
                          </div>
                        </div>
                ...
                <PaginationControls
                  currentPage={recurrentPage}
                  totalPages={Math.ceil(highRiskOperators.length / dynamicItemsPerPage.recurrent)}
                  onPageChange={setRecurrentPage}
                />
              </div>
            </div>
            ...
            <div className={`flex-1 rounded-2xl border p-[2vh] overflow-hidden flex flex-col ${getCardBg()}`}>
              <div className="flex items-center justify-between mb-[1.5vh] shrink-0">
                ...
                <span className="bg-orange-600 text-white text-[1rem] font-bold px-3 py-1 rounded-full">
              ...
              <div className="flex-1 flex flex-col justify-between overflow-hidden" ref={highRiskListContainerRef}>
                <div className="flex-1 space-y-2">
                  {highFreqZones.length > 0 ? (
                    paginate(highFreqZones, highRiskPage, dynamicItemsPerPage.highRisk).map((zone, idx) => (
                      <div key={`${zone.location}-${idx}`} className={`px-3 py-2 rounded-xl border flex justify-between items-center ${darkMode ? 'bg-slate-900/50 border-slate-700' : 'bg-white border-slate-200'}`}>
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="w-3 h-3 rounded-full bg-red-500" />
                            <span className={`text-[clamp(1rem,1.2vh,1.4rem)] font-bold ${darkMode ? 'text-white' : 'text-slate-800'}`}>
                              {zone.location}
                            </span>
                          </div>
                          <div className={`text-[0.9rem] ml-5 ${darkMode ? 'text-slate-300' : 'text-slate-500'}`}>
                            Events: <span className="font-mono font-bold text-slate-400">{zone.eventsCount}</span>
                          </div>
                        </div>
                        <div className={`text-[0.9rem] px-3 py-1 rounded border flex items-center gap-2 font-medium ${
                          zone.area === 'Mining'
                ...
                <PaginationControls
                  currentPage={highRiskPage}
                  totalPages={Math.ceil(highFreqZones.length / dynamicItemsPerPage.highRisk)}
        ...
        <div className="flex-1 flex flex-col gap-[2vh] min-w-[320px] lg:min-w-[400px] h-full">
          <div className={`p-[2vh] rounded-2xl border flex flex-col justify-center shrink-0 h-[16vh] min-h-[140px] ${getCardBg()}`}>
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className={`text-[clamp(1rem,1.2vh,1.5rem)] font-bold uppercase tracking-wider ${darkMode ? 'text-white' : 'text-slate-500'}`}>
                Global Filter
              </span>
            </div>
            <div className={`flex p-2 rounded-xl ${darkMode ? 'bg-slate-900' : 'bg-slate-200'}`}>
              {['All', 'Mining', 'Hauling'].map((area) => (
                <button
                  ...
                  className={`flex-1 py-[1.5vh] text-[clamp(1rem,1.5vh,1.6rem)] font-bold rounded-lg transition-all ${
                    selectedArea === area
                      ? darkMode
          ...
          <div className={`flex-1 flex flex-col rounded-2xl border overflow-hidden ${getCardBg()}`}>
            <div className="p-[2.5vh] border-b border-inherit shrink-0">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className={`font-bold text-[clamp(1.5rem,2vh,2.5rem)] ${darkMode ? 'text-white' : 'text-slate-800'}`}>
                    Active Alerts
                  </h3>
                </div>
                <span className={`text-lg font-bold ${darkMode ? 'text-white' : 'text-slate-600'}`}>{filteredAlerts.length} Total</span>
              </div>
              <div className={`flex justify-between items-center text-[1rem] ${darkMode ? 'text-slate-300' : 'opacity-60'}`}>
            ...
            <div className="flex-1 flex flex-col justify-between overflow-hidden p-[2vh]" ref={activeFatigueListContainerRef}>
              <div className="space-y-[1.5vh]">
                {filteredAlerts.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-slate-500 opacity-60">
                    ...
                    <p className="text-xl text-center">All clear</p>
                    {selectedLocationFilter && (
                      <button
                  ...
                  paginate(filteredAlerts, activeFatiguePage, dynamicItemsPerPage.activeFatigue).map((alert) => (
                    <div
                      key={alert.alarmId}
                      className={`relative p-[1.5vh] rounded-xl border transition-all hover:scale-[1.01] cursor-pointer group ${
                        darkMode
                          ? 'bg-slate-800 border-l-8 border-l-red-500 border-y-slate-700 border-r-slate-700 hover:border-slate-500'
                          : 'bg-white border-l-8 border-l-red-500 border-y-slate-200 border-r-slate-200 shadow-sm hover:shadow-md'
                      }`}
                      onClick={() => setSelectedAlert(alert)}
                    >
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-3">
                          <span className="px-2 py-1 rounded-md text-[0.9rem] font-bold uppercase bg-red-600 text-white">
                            {alert.alarmType || 'Fatigue'}
                          </span>
                          <span className={`text-[1rem] font-mono ${darkMode ? 'text-slate-300' : 'text-slate-500'}`}>
                            {formatTime(alert.openedAt)}
                          </span>
                        </div>
                        <span className="text-[0.9rem] text-red-500 font-bold flex items-center gap-2 animate-pulse">
                          <AlertTriangle size={14} /> ACTIVE
                        </span>
                      </div>
                      <div className="flex items-center gap-3 mb-2">
                        <div className={`p-2 rounded-xl ${darkMode ? 'bg-slate-700' : 'bg-slate-100'}`}>
                          <Truck className={`w-[3vh] h-[3vh] ${darkMode ? 'text-slate-300' : 'text-slate-600'}`} />
                        </div>
                        <div>
                          <h4 className={`font-bold text-[clamp(1.2rem,1.5vh,1.8rem)] ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                            {alert.unit}
                          </h4>
                          <p className={`text-[1rem] ${darkMode ? 'text-slate-300' : 'text-slate-500'}`}>
                            {alert.operator} • {alert.eventCount}x Today
                          </p>
                        </div>
                      </div>
                      <div className={`text-[1rem] p-2 rounded-lg flex justify-between items-center ${darkMode ? 'bg-slate-900' : 'bg-slate-100'}`}>
                        <div className={`flex items-center gap-2 font-medium ${darkMode ? 'text-slate-300' : 'text-slate-500'}`}>
                          <Map size={18} />
                          <span>{alert.location}</span>
                        </div>
                        <div className="flex items-center gap-2 text-red-400 font-mono font-bold animate-pulse">
                          <Clock size={18} />
                          <span>+{getOpenDurationValue(alert.openedAt, alert.openMinutes)}m</span>
                        </div>
                      </div>
                    </div>
                  ))
              ...
              <PaginationControls
                ...
                totalPages={Math.ceil(filteredAlerts.length / dynamicItemsPerPage.activeFatigue)}
