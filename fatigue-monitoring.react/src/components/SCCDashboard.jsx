...
const SCCDashboard = () => {
  ...
  const { data: sseData, connectionStatus, error: sseError } = useSSE(`${API_BASE_URL}/api/sse/stream`);
  ...
  // Show notification on new alerts
  useEffect(() => {
    if (sseData?.activeAlerts) {
      const newAlerts = sseData.activeAlerts.slice(0, 1); // Get the newest alert
  ...
  const stats = useMemo(() => {
    if (!sseData?.stats) return { totalToday: 0, followedUpToday: 0, activeOpen: 0 };
    
    const areaStats = sseData.stats[selectedArea] || { totalAlarms: 0, followedUp: 0, waitingFollowUp: 0 };
    return {
      totalToday: areaStats.totalAlarms,
      followedUpToday: areaStats.followedUp,
      activeOpen: areaStats.waitingFollowUp
    };
  }, [sseData, selectedArea]);

  const alerts = useMemo(() => {
    if (!sseData?.activeAlerts) return [];
    
    return sseData.activeAlerts.map(alert => ({
      id: alert.id,
      unit: alert.unit,
      ...
      time: new Date(alert.time).toLocaleTimeString('en-GB', { hour12: false }),
      status: alert.status,
      speed: alert.speed,
      count: alert.count,
      latitude: alert.latitude,
      longitude: alert.longitude,
      openDurationMinutes: alert.openDurationMinutes
    }));
  }, [sseData]);
  ...
  const overdueAlerts = useMemo(() => {
    if (!sseData?.delayedAlerts) return [];
    
    const delayed = sseData.delayedAlerts;
    if (selectedArea !== 'All') {
      return delayed.filter(a => a.area === selectedArea);
    }
    return delayed;
  }, [sseData, selectedArea]);

  const locationStats = useMemo(() => {
    if (!sseData?.areaDistribution) return { Mining: {}, Hauling: {} };
    
    const stats = { Mining: {}, Hauling: {} };
    const dist = sseData.areaDistribution;
    
    if (dist.Mining) {
      dist.Mining.forEach(item => {
        stats.Mining[item.location] = item.alertCount;
      });
    }
    
    if (dist.Hauling) {
      dist.Hauling.forEach(item => {
        stats.Hauling[item.location] = item.alertCount;
      });
    }
  ...
  const highRiskOperators = useMemo(() => {
    ...
    const units = sseData.recurrentUnits;
    if (selectedArea !== 'All') {
      return units.filter(u => u.area === selectedArea);
    }
    return units;
  }, [sseData, selectedArea]);
  ...
  const highFreqZones = useMemo(() => {
    ...
    const areas = sseData.highRiskAreas.map(a => ({
      location: a.location,
      count: a.count,
      area: a.area
    }));
    ...
    if (selectedArea !== 'All') {
      return areas.filter(a => a.area === selectedArea);
    }
    return areas;
  }, [sseData, selectedArea]);
  ...
  const ConnectionStatusIndicator = () => {
    ...
    return (
      <div className={`flex items-center gap-3 px-[1.5vw] py-[1vh] rounded-full border ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-300 shadow-sm'}`}>
        <div className={`w-3 h-3 lg:w-4 lg:h-4 rounded-full ${getStatusColor()}`}></div>
        <span className={`text-[clamp(1rem,1.2vh,1.5rem)] font-semibold whitespace-nowrap ${darkMode ? 'text-slate-200' : 'text-slate-700'}`}>
          SSE: {getStatusText()}
        </span>
      </div>
    );
                        ...
                        <div className="flex flex-col gap-[2vh] h-full">
                            ...
                            <div className="flex-1 relative rounded-2xl bg-black border-4 border-slate-700 overflow-hidden group">
                                ...
                                <div className="absolute top-[25%] left-[30%] right-[30%] bottom-[25%] border-4 border-red-500/80 rounded-xl z-30 shadow-[0_0_20px_rgba(239,68,68,0.5)] animate-pulse flex flex-col items-center justify-end pb-8">
                                     <div className="bg-red-600 text-white text-xl font-bold px-6 py-3 rounded-lg flex items-center gap-3 shadow-lg">
                                        <EyeOff size={24} /> {selectedAlert.type}
                                     </div>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4 shrink-0 h-[12vh]">
                                 ...
                                 <div className={`p-6 rounded-xl border flex flex-col justify-center ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-300'}`}>
                                    ...
                                    <span className="font-bold text-4xl text-red-500">{selectedAlert.type}</span>
                                 </div>
                            </div>
                        </div>
                        {/* RIGHT */}
                        <div className="flex flex-col gap-[2vh] h-full">
                            ...
                            <div className={`p-6 rounded-xl border shrink-0 flex items-center justify-between ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-300'}`}>
                                <div>
                                    ...
                                    <span className="font-mono text-2xl font-medium text-blue-500">Lat: {selectedAlert.latitude?.toFixed(3)}, Long: {selectedAlert.longitude?.toFixed(3)}</span>
                                ...
                                paginate(overdueAlerts, delayedPage, ITEMS_DELAYED).map((alert) => (
                                    <div key={alert.id} onClick={() => setSelectedAlert(alert)} className={`relative group p-3 rounded-xl border flex justify-between items-center cursor-pointer transition-all hover:bg-red-500/10 hover:border-red-400 ${darkMode ? 'bg-slate-900 border-red-500/30' : 'bg-white border-red-200 shadow-sm'}`}>
                                        ...
                                        <div className="text-right">
                                            <div className="text-[1.2rem] font-black text-red-500 font-mono">+{alert.openDurationMinutes}m</div>
